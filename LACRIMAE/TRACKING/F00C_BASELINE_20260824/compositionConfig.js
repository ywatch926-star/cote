...
export function getCompositionConfig(codex = {}, session = {}) {
  ...
  return {
    ...
    video_scale: Number(raw.video_scale ?? 1),
    video_position_x: Number(raw.video_position_x ?? 0),
    video_position_y: Number(raw.video_position_y ?? 0),
...
export function rotationForSequence(config, sequenceIndex, frame, fps = 30, durationInFrames = 300) {
  if (config.rotation_mode === 'static' || config.rotation_mode === 'fixed') {
    return config.rotation_deg || 0;
  }
