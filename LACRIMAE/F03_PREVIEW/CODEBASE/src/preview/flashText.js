...
export function activeFlashTextUnit(texts, frame) {
  const units = normalizeFlashTextUnits(texts);
  return units.find((unit) => frame >= unit.start_frame && frame < unit.start_frame + unit.duration_frames) || null;
}
...
export function flashTextStyle(unit, frame, style = {}) {
  ...
  return {
    ...
    fontSize: `${unit.font_size || style.size_title || 96}px`,
    fontWeight: 900,
    lineHeight: 0.95,
    letterSpacing: unit.letter_spacing || '-0.02em',
    textAlign: 'center',
